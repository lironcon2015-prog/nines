...
async function boot() {
  ...
  /* מאיפה מגיע התוכן. gate:"pack" באינדקס אומר שהאתר הזה אינו מגיש את
     החוברות שלו — הן מגיעות מחבילה שנטענה למכשיר. בלי חבילה מסך הבית
     הוא מסך הטעינה, ולא שגיאה.

     החבילה נקראת רק כשהאינדקס של האתר מבקש אותה. שלוש הגרסאות יושבות על
  ...
  const list = stored ? null : (site.gate === 'pack' ? [] : site.booklets);
  const booklets = stored ? stored.booklets : (await Promise.all(
    list.map(b => getJSON(b.file).catch(err => { console.error(err); return null; }))
  )).filter(Boolean);
  ...
  /* לגרסה שקדמה לחוברות הייתה חוברת אחת בלבד; legacyBooklet אומר לאיזו
     ...
     כשהוא נקוב ואינו כאן — גרסה לתפקיד אחד — אין למי להגר, ולסמן אותם
     על חוברת אחרת פירושו להציג לילד כלמוד תרחיש שלא ראה. */
  const legacyTarget = index.legacyBooklet
    ? booklets.find(b => b.id === index.legacyBooklet)
    : booklets[0];
  if (legacyTarget) migrateLegacy(legacyTarget.id, legacyTarget.scenarios.map(s => s.id));
  ...
  function show(route) {
    ...
    if (route.view === 'quiz') {
      ...
      screens('library');
      renderLibrary(home, booklets, formations, openBooklet, openQuiz, openTrain, index.version, pack);
    }
    window.scrollTo(0, 0);
  }
