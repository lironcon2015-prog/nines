...
/* --- הצד שבו משחקים: R כברירת מחדל, L משקף את כל התרחישים ---
   הגרסה המלאה והגרסאות לתפקיד יושבות על אותו דומיין וחולקות
   localStorage, אבל הצד הוא העדפה של הגרסה ולא של המכשיר: הכנף נפתח על
   ימין והמגן על שמאל, ובמפתח משותף כל מעבר בין הכתובות היה הופך אותו.
   הגרסה המלאה נשארת בלי סיומת, כדי שהבחירה שכבר נשמרה לא תיעלם. */
const SCOPE = (document.querySelector('meta[name="app-scope"]') || {}).content || '';
const SIDE_KEY = SCOPE ? 'k8:side:' + SCOPE : 'k8:side';
